...
describe('useExcelStore', () => {
  ...
  it('loadTables: populates tables and loads a sheet', async () => {
    ...
    expect(store.sheet!.rows.length).toBeGreaterThan(0)
  })
  ...
  it('getCellDisplay returns a non-empty string for row 0, col 0 after loading', async () => {
    await useExcelStore.getState().loadTables()
  ...
  it('getFieldAt(0) returns the primary text field with id 1 after loading', async () => {
    await useExcelStore.getState().loadTables()
